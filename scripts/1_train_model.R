...
# select variables
top_vars <- names(sort(abs(univ_cox), decreasing = TRUE)[1:9])  # Keep top 30
filtered_data <- filtered_data[, c("Recurrence", top_vars)]
# top 30 variables already (unsurprisingly) includes RFS_time, no need to add again
...
decision_matrix <- table(Predicted = knowles_imputed$risk_group, 
